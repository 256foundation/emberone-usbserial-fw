    ...
    fn from_bytes(buf: &[u8]) -> Result<Self, CommandError> {
        ...
        match buf[2] {
            PSU_COMMAND => Ok(Self {
                id,
                bus: buf[1],
                inner: CommandInner::Psu(psu::Command::from_bytes(&buf[3..])?),
            }),
            I2C_COMMAND => Ok(Self {
                id,
                bus: buf[1],
    ...
    pub async fn run(&mut self) {
        loop {
            ...
            let res = match cmd.inner {
                CommandInner::Psu(cmd) => cmd.handle(&mut self.psu_i2c).await,
